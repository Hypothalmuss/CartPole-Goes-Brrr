"""
Train a PPO agent on the custom double-pendulum-cart environment.

Usage:
    python train.py                          # train from scratch
    python train.py --mode eval              # load saved model & watch it
"""
...
ENT_COEF = 0.03   # was 0.01 — temporary bump to test exploration hypothesis
# ENT_COEF = 0.01  # previous default
...
def train():
    ...
    checkpoint_callback = CheckpointCallback(
        ...
        save_path=OUT_DIR,
        name_prefix="ppo_double_pendulum",
    )
    ...
    final_path = OUT_DIR / "ppo_double_pendulum_final.zip"
...
def evaluate():
    model_path = OUT_DIR / "ppo_double_pendulum_final.zip"
    if not model_path.exists():
        # try checkpoint
        checkpoints = sorted(OUT_DIR.glob("ppo_double_pendulum_*_steps.zip"))
        if not checkpoints:
